...
assert.match(ui, /ROLE-003/, 'guided demo must default to ROLE-003');
assert.match(ui, /guidedSteps/, 'guided demo must be lightweight application state');
assert.equal((ui.match(/\['/g) ?? []).length >= 11, true, 'guided demo must include eleven steps');
...
assert.equal(growth.active.confidence, 79, 'fixture should raise active readiness to 79%');
...
assert.deepEqual([technologyChange.before_classification, technologyChange.before_confidence, technologyChange.after_classification, technologyChange.after_confidence], ['adjacent', 78, 'direct', 86], 'What Changed must reflect the deterministic transition change');
...
console.log('COMPANION_EXPERIENCE_TEST=PASS guided=11 deterministic=78-to-79 privacy=preserved');
